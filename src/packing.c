  ...
  do { // g loop
    ...
    do { // NC loop
      ...
      for (size_t kr_block_start = 0; kr_block_start < round_up_po2(kc, kr * 2); kr_block_start += kr * 2) {
        ...
        // Better than spills?
        int32_t ksum[nr];
        float ksum_muls[nr];  // Multipliers for ksum
        float scales[nr];
        const uint8_t* ip[nr];
        uint8_t* op[nr];
        ...
        for (size_t nr_block_offset = 0; nr_block_offset < nr; nr_block_offset++) {
          ...
          op[nr_block_offset] = packed_weights + nr_block_offset * kr;  // 2 nibbles
...
inline bool can_use_xnn_pack_qs8_qc4w_gemm_bl_goi_w_nr8_kr4(
